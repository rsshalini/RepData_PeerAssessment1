...
```
The mean and median number of steps is `r meanQ1` and `r medianQ1` respectively.
...
```
The 5-minute time interval at which the the average daily steps is maximum occurs at `r maxinterval` (i.e.At 0835 hours).
...
```
The total number of missing values in the dataset are `r nrowsNa`.
...
```
The mean and median number of steps is `r meanQ3` and `r medianQ3` respectively.

In comparison with the mean and median in Question1, the mean is the same and the median is increased from `r medianQ1` to `r medianQ3`. The median is now equal to the mean.
...
```{r adding day variable, echo=TRUE}
